...
def test_and_write(sess, 
    ...
    if output_nnet:
        ffnet = write_nnet(sess, output_ops, feed_dict, activation_fn, activation_type, thetas_v, theta_dot_hats_v, theta_dot_LBs, theta_dot_UBs, network_dir, nsteps, f_id, verbose)
...
def write_nnet(sess, output_ops, feed_dict, activation_fn, activation_type, thetas_v, theta_dot_hats_v, theta_dot_LBs, theta_dot_UBs, network_dir, nsteps, f_id, verbose):
    output_op_names = [op.name for op in output_ops]
    output_graph_def = graph_util.convert_variables_to_constants(
    sess, # sess used to retrieve weights
    sess.graph.as_graph_def(), # graph def used to retrieve nodes
    ...
    ff_feed_dict = {
            ff_input: np.vstack([feed_dict[i] for i in input_list[7:]]) # strip off the "import" part of every input in input_list
        }
    ff_output_tensors_v = sess.run(ffnet, feed_dict=ff_feed_dict)
    if verbose:
        print("condensed output: ", output_tensors_v)
    # check equivalency
    assert(all(abs(np.array(thetas_v).flatten() - output_tensors_v[0:nsteps].flatten()))<1e-4) # thetas
    assert(all(abs(np.array(theta_dot_hats_v).flatten() - output_tensors_v[nsteps:2*nsteps].flatten()))<1e-4) # theta dot hats
    assert(all(abs(np.array(theta_dot_LBs_v).flatten() - output_tensors_v[2*nsteps:3*nsteps].flatten())<1e-4)) # theta dot LBs
    assert(all(abs(np.array(theta_dot_UBs_v).flatten() - output_tensors_v[3*nsteps:].flatten())<1e-4))# theta dot UBs
    #
    #
    # Write to .nnet file
    ...
    inMins = [0.]*num_inputs
    inMaxs = [0.]*num_inputs
    fileName = os.path.join(network_dir, "overrapprox_const_dyn_"+nsteps+"_steps_"+f_id+".nnet")
    ...
    write_metadata(input_list, imported_output_op_names, network_dir, f_id)
    return ffnet
